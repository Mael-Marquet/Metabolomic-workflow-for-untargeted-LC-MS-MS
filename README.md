...
| cluster      | 2.1.8.1 | Clustering algorithms                     |
...
| future       | 1.70.0  | Future parallel processing                |
| future.callr | 1.0.0   | Callr backend for future                  |
| parallelly   | 1.47.0  | Parallel backend utilities                |
...
| igraph       | 2.3.3   | Graph-based feature grouping              |
...
| ComplexHeatmap | 2.26.0  | Advanced heatmaps                            |
...
A `renv.lock` file is provided to guarantee an exact reproduction of the
package environment used during development.

The repository includes three renv-related files that must stay together
:
...
These are all included when you clone the repository — no manual setup
needed.
...
> This step is optional — the `check_and_load()` function handles
> installation independently. Use `renv::restore()` only if you need a
