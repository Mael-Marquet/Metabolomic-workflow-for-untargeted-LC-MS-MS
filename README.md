...
- Possibilities : GNPS2, your homemade base, Compound Discoverer 
...
This workflow requires **R 4.5.2** (2025-10-31 ucrt).
...
[CRAN](https://cran.r-project.org/bin/windows/base/old/4.5.2) (choose your operating system —
Windows, macOS, or Linux). Use the latest version (R 4.5.2).

**2. Install Rtools (Windows only)**
 
Some packages (particularly Bioconductor ones) may not have a
pre-compiled binary available and need to be built from source on
Windows. Rtools provides the required compiler toolchain.
 
Download and install [Rtools45](https://cran.r-project.org/bin/windows/Rtools/)
(matching R 4.5.x). During installation, keep the default option to add
Rtools to the system PATH.
 
*(macOS and Linux users can skip this step — compilers are usually
already available or installed automatically.)*
