...
check_and_load("ggsignif", "0.6.4", FALSE, "Significance Brackets for 'ggplot2'")
...
check_and_load("MsBackendMgf", "1.18.0", TRUE, "Import/export of MS/MS spectra in MGF format")
check_and_load("MsBackendMsp", "1.14.0", TRUE, "Import/export of MS/MS spectra in MSP format")
...
preprocessing_method_choice_res <- select_preprocessing_method(param_folder, data_folder)

if(!isTRUE(preprocessing_method_choice_res$is_EB)){
  ...
  peak_picking_results <- perform_peak_picking_centwave(file_list_tot = file_list_tot,
  ...
  gap_filling_results <- perform_gap_filling(xdata_grouped, data_folder, param_folder)
  ...
  saveRDS(feature_metadata, file = paste0(data_folder, "/1_Pre-processing/feature_metadata_plus_indexs_XCMS.rds")) # Saving peak indices as a list
  
  write.csv(feature_metadata %>% select(-peakidx),   # Removal of peak indices (list form can not be exported)
            file = paste0(data_folder, "/1_Pre-processing/feature_metadata_Wadduct_XCMS.csv"), row.names = TRUE)
  
  write.csv(raw_intensity_matrix, file = paste0(data_folder, "/1_Pre-processing/raw_intensity_matrix_XCMS.csv"), row.names = TRUE)
  
  if(!is.null(export_MS2_spectra_res)){
    write.table(raw_intensity_matrix_GNPS2, file = paste0(data_folder, "/1_Pre-processing/MS2_export/raw_intensity_matrix_GNPS2_XCMS.txt"), sep = "\t",
  ...
  save.image(file = file.path(Save_secu_folder, "After_Pre_Processing_XCMS.RData"), compress = "gzip", safe = T)
  ...
  colnames(raw_intensity_matrix) <- str_remove(colnames(raw_intensity_matrix), "\\.mzML Peak area$")
  
  raw_intensity_matrix <- raw_intensity_matrix[, c(blank_names, qc_names, bio_names)]
  
  # ms2 consensus : .mgf
  
  ms2_consensus <- preprocessing_method_choice_res$ms2_consensus
  
  # feature_metadata creation
  
  feature_metadata <- preprocessing_method_choice_res$aligned_features %>%
    dplyr::rename(rtmed = `row retention time`, mzmed = `row m/z`) %>% 
  ...
  write.csv(feature_metadata,
            file = paste0(data_folder, "/1_Pre-processing/feature_metadata_EB.csv"), row.names = TRUE)
  
  write.csv(raw_intensity_matrix, file = paste0(data_folder, "/1_Pre-processing/raw_intensity_matrix_EB.csv"), row.names = TRUE)
  ...
  save.image(file = file.path(Save_secu_folder, "After_Pre_Processing_EB.RData"), compress = "gzip", safe = T)
...
if (!is.null(RT_results)) {
  step_df <- RT_results$filtered_data
}


##########################################################################################################
############################       Filter data by Blank Filtering     ###############################
...
if (!is.null(outlier_results)) {
  step_df <- outlier_results$filtered_data
...
##########################################################################################################
###################################  Pre-LOESS/Normalization permissive CV filtering (on QC)  ####################

# Function

CV_results_pre_LOESS <- apply_exploratory_CV_filter(
  input_data = step_df,
  qc_names = qc_without_outliers,
  metadata_table = metadata_table,
  param_folder = param_folder,
...
outlier_results2 <- filter_sample_outliers(
  input_data = step_df,
  blank_names = blank_names,
  bio_names = bio_without_outliers,
  qc_names = qc_without_outliers,
...
if(!isTRUE(preprocessing_method_choice_res$is_EB)){
  ...
  GNPS_annot_res <- annotate_GNPS_FBMNW(
    param_folder          = param_folder,
    data_folder           = data_folder
  )
  ...
  GNPS_annot_res <- annotate_GNPS_EB(
    param_folder          = param_folder,
    data_folder           = data_folder
  )
...
if (has_annotation) {
  ...
  write.csv(Annotation_summary_table, file = paste0(data_folder, "/3_Annotation/Annotation_summary_table.csv"),
            row.names = FALSE)
  write.csv(Final_output_annotated, file = paste0(data_folder, "/3_Annotation/Final_output_annotated.csv"),
            row.names = TRUE)
  write.csv(Final_output_annotated_complete, file = paste0(data_folder, "/3_Annotation/Final_output_annotated_complete.csv"),
            row.names = TRUE)
  write.csv(pre_scaled_matrix, file = paste0(data_folder, "/3_Annotation/pre_scaled_matrix.csv"),
            row.names = TRUE)
