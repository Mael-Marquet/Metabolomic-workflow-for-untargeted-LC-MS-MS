...
The workflow covers the full analysis chain : from `.mzML` file
import to statistical analysis, with interactive Shiny-based guidance at
key steps.
...
-   Currently performed using the export table
from Compound Discoverer
...
A `renv.lock` file is provided to guarantee an exact reproduction of the package environment used during development.
...
These are all included when you clone the repository — no manual setup needed.
...
-   **Do not modify the code during execution** unless you know what you are
    doing.
-   Always press **Submit** when the figure or result you want to save
    is displayed.
-   Outputs are **saved automatically**.
-   Safety R Data Checkpoints can be used to re-run specific
    steps without restarting from scratch.
    
-   When a Shiny page becomes **grayed out**, the current step is finished.
    Close or leave the page and return to the R code to proceed with the next step.
...
-   Feature annotation is currently limited to the **Compound
    Discoverer** export table (proprietary software, Thermo Fisher
    Scientific). Integration with open-source annotation tools (e.g.,
    MZmine) is planned for future versions.
